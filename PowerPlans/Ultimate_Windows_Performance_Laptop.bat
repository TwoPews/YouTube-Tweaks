...
:: Ultimate Windows Performance (Laptop)
for /f "tokens=3" %%G in ('powercfg -duplicatescheme SCHEME_BALANCED ^| findstr /i "Power Scheme GUID"') do set "GUID=%%G"
if "%GUID%"=="" (
  echo Failed to create power plan. Run as Administrator.
  pause
  exit /b 1
)
powercfg -changename %GUID% "Ultimate Windows Performance (Laptop)" "Max performance on AC, balanced on battery"
...
powercfg -setdcvalueindex %GUID% 54533251-82be-4824-96c1-47b60b740d00 bc5038f7-23e0-4960-96da-33abaf5935ec 85
powercfg -setactive %GUID%
echo Created and activated: Ultimate Windows Performance (Laptop)
pause
endlocal
